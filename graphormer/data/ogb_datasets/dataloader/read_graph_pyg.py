...
def read_graph_pyg(raw_dir, add_inverse_edge=False, additional_node_files=[], additional_edge_files=[], binary=False):
    ...
    for graph in tqdm(graph_list):
        ...
        g.edge_index = torch.from_numpy(graph['edge_index']).to(torch.int64)
        ...
        if graph['node_label'] is not None:
            # g.y = torch.from_numpy(graph['node_label']).to(torch.float32)
            g.y = torch.from_numpy(graph['node_label']).to(torch.int64)
